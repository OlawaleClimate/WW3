module W3SNLMLMD
  ...
  !use, intrinsic :: iso_fortran_env, only : sp => real16
  ...
  subroutine ml_init(mean_data, std_data, out_mean_data, out_std_data, rankk,  model)
    ...
    rank = mod(rankk-1,4)
    ! Load ML model
    device_name = torch_kCUDA

    model_filename = '/pscratch/sd/o/olawale/Postdoc_NCAR/ML_models/model_fac_std_NL2_L3_3072_300ep_gpu_ts.pt'

    if (device_name .eq. torch_kCUDA) then
          model = torch_module_load(trim(model_filename), device_type=device_name, device_index=rank)
    else
          model = torch_module_load(trim(model_filename), device_type=device_name)
    end if
  ...
  subroutine ml_routine(DW, CG, VA, VNL_ML, mean_data, std_data, out_mean_data, out_std_data, rankk, model)
    ...
    integer :: JSEA, ISEA, IFR, ITH, ISP,rank
    real :: CONX
    integer, parameter :: in_dims = 2, out_dims =2
    integer :: in_layout(in_dims), out_layout(out_dims),device_name
    ...
    if (device_name .eq. torch_kCUDA) then
        in_tensors(1) = torch_tensor_from_array(input_array2, in_layout,device_name, device_index=rank)
    else
         in_tensors(1) = torch_tensor_from_array(input_array2, in_layout,device_name)
    end if
